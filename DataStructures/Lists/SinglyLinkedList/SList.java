...
public class SList <E extends Comparable<E>> {
    ...
    public void insertFront(E newItem) { // 연결리스트의 맨 앞에 노드 삽입
        head = new Node<E>(newItem, head); // head를 기존 노드를 참조하는 새 노드로 교체
        size++;
    }

    public void insertAfter(E newItem, Node<E> p) { // p 노드 뒤에 새 노드 삽입
        p.setNext(new Node<E>(newItem, p.getNext())); // 기존에 p 노드가 참조하던 노드를 참조하는 새로운 노드를 생성하고, p 노드가 해당 노드를 참조하도록 설정
        size++;
    }
    ...
    public void deleteAfter(Node<E> p) {
        ...
        Node<E> t = p.getNext(); // p가 참조하는 노드를 t에 저장
